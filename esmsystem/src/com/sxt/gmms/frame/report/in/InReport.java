...
public class InReport {
	 ...
	 * Create the frame.
	 */
	public InReport(MainFrame mf) {
		InItemDao itemDao = new InItemDao();
		List<InStorageItem> itemList = itemDao.loadInStorageReport();

		// 数据集
		DefaultCategoryDataset dataSet = new DefaultCategoryDataset();
		for (InStorageItem item : itemList) {
			dataSet.addValue(item.getInItemQty(), "", item.getGoods()
					.getGoodsName());
		}

		JFreeChart chart = ChartFactory.createBarChart3D("进货统计图", "产品名称",
				"进货量", dataSet, PlotOrientation.VERTICAL, true, true, true);
		...
		TextTitle title = new TextTitle("进货统计图", font);
		// 副标题
		TextTitle subtitle = new TextTitle("总的进货量", new Font("黑体", Font.BOLD,
				12));
		...
		String path = "image/report/in.png";
		...
		try {
			...
			InReportFrame inReportFrame = new InReportFrame(mf, path);
			mf.desktopPane.add(inReportFrame);
			inReportFrame.setSelected(true);
		} catch (Exception e) {
			e.printStackTrace();
		} 
	}
