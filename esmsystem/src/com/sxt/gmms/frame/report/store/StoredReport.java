package com.sxt.gmms.frame.report.store;
...
public class StoredReport {
	 ...
	 * Create the frame.
	 */
	public StoredReport(MainFrame mf) {
		StoredDao itemDao = new StoredDao();
		List<Stock> itemList = itemDao.loadStockInfo();

		// 数据集
		DefaultCategoryDataset dataSet = new DefaultCategoryDataset();
		for (Stock item : itemList) {
			dataSet.addValue(item.getStockQty(), "", item.getGoods()
					.getGoodsName());
		}

		JFreeChart chart = ChartFactory.createBarChart3D("库存统计图", "产品名称", "库存量",
		...
		TextTitle title = new TextTitle("库存统计图", font);
		// 副标题
		TextTitle subtitle = new TextTitle("总的库存", new Font("黑体", Font.BOLD, 12));
		...
		String path = "image/report/stock.png";
		...
		try {
			...
			StoredReportFrame storedReportFrame = new StoredReportFrame(mf, path);
			mf.desktopPane.add(storedReportFrame);
			storedReportFrame.setSelected(true);
		} catch (Exception e) {
			e.printStackTrace();
		} 
	}
